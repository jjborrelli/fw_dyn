...
dyn2 <- Crmod(Adj = nm1, t = 1:500, G = G.i, method = conres, FuncRes = Fij, K = 1, x.i = .5, yij = 6, eij = 1, xpar = .2, B.o =.5, plot = TRUE)
```

The dynamics of the web can be visualized: 

```{r webdyn, fig.width=7, fig.height=6, fig.show='animate',eval = F}
lay <- matrix(runif(nrow(nm1)*2), ncol = 2)
s <- matrix(0, nrow = nrow(dyn2), ncol = ncol(nm1))
par(mar = rep(.1, 4))
for(i in 1:500){
  strength <- melt(Fij(dyn2[i,-1], nm1, .5, .2))
  lstr <- strength[,3][strength[,3] > 0]
  s[i,c(which(dyn2[i,-1] > 0))] <-log(dyn2[i, c(which(dyn2[i,] > 0)[-1])])+abs(min(log(dyn2[i, c(which(dyn2[i,] > 0)[-1])])))
  plot.igraph(graph.adjacency(nm1), vertex.size = s[i,], edge.width = lstr , edge.arrow.size = .5, layout = lay)
}
```
