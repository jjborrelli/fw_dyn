...
RESULT <- foreach(i = 1:2) %dopar%{
  source("./Rscripts/MODELfunctions.R")
  test <- CRmod.gen(200, .1, modTYPE[i], xpar = 1)
  test2 <- CRmod.gen(200, .1, modTYPE[i], xpar = .2)
  ...
  return(list(z.results, z.results2, results, results2))
}
...
for(i in 101:200){
  test <- CRmod.gen(200, .1, "niche", xpar = 1)
  test2 <- CRmod.gen(200, .1, "niche", xpar = .2)
  z.results[[i]] <- data.frame(q = 1, iter = i, test$z)
  z.results2[[i]] <- data.frame(q = .2, iter = i, test2$z)
  results[[i]] <- data.frame(q = 1, iter = i, test$wp)
  results2[[i]] <- data.frame(q = .2, iter = i, test2$wp)
  print(i)
}
